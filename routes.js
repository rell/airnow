...
function query_api_predictions(f_init_date = null, f_predict_date = null, f_predict_time = null, sitename = null, station_lat = null, station_lon = null, stationid = null, nw_lat = null, nw_lon = null, se_lat = null, se_lon = null) {
...
var getStations = function (req, res) {
    ...
    connection.query(query, function (err, rows, fields) {
        if (err) console.log(err); else res.json(rows)
    })
}
...
var getStationPlot = function (req, res) {
    ...
    connection.query(query, [1, 2, 3, 4], function (err, rows, fields) {
        if (err) console.log(err); else {
            var return_data = {
                'station_info': rows[0][0], 'pm25_hour': rows[1], 'predictions': rows[2], 'grouped_predictions': rows[3]
            }
...
var queryPredictions = function (req, res) {
    var query = query_api_predictions(req.query.f_init_date, req.query.f_predict_date, req.query.f_predict_time, req.query.sitename, req.query.station_lat, req.query.station_lon, req.query.stationid, req.query.nw_lat, req.query.nw_lon, req.query.se_lat, req.query.se_lon);
    connection.query(query, function (err, rows, fields) {
        if (err) console.log(err); else res.json(rows)
    })
}
...
module.exports = {
    get_main: getMain, //get_test: getTest,
