const config = require('./db-config-goes.js');
...
// get one-hour observation values from all stations
function query_all_stations_one_hour_goes(year, month, day, hour) {
    return `WITH hour_filter as (SELECT DISTINCT * FROM PM25_Predictions\
          WHERE YEAR(UTC) = ${year} AND MONTH(UTC) = ${month} AND DAY(UTC) = ${day} AND HOUR(UTC) = ${hour})\
          SELECT * FROM hour_filter RIGHT JOIN Stations
          on hour_filter.stationid = Stations.stationid
          ORDER BY sitename`;
...
// get all observation values from one station
function query_one_station_all_hours_goes(stationid,year,month,day){
  return `SELECT * FROM PM25_Predictions WHERE YEAR(UTC) = ${year} AND MONTH(UTC) = ${month} AND DAY(UTC) = ${day} AND stationid = '${stationid}'`
}

function query_date(){
  return `select max(UTC) as 'UTC'
from PM25_Hourly`
}
...
var getHourlyGoes = function(req, res) {
  var [year, month, day, hour] = [req.query['year'], req.query['month'], req.query['day'], req.query['hour']]
  var query = query_all_stations_one_hour_goes(year, month, day, hour)
  connection.query(query, function(err, rows, fields) {
    if (err) console.log(err);
    else res.json(rows)
  })
...
var getStationPlot = function(req, res) {
  ...
  var query =  `${query_one_station(stationid)};
	        ...
	        ${query_one_station_all_hours_goes(stationid,init_year,init_month,init_day)};`
  var return_data = {};
  connection.query(query, [1,2,3], function(err, rows, fields) {
    if (err) console.log(err);
    else {
      var return_data = {'station_info': rows[0][0], 'pm25_hour': rows[1],'goes':rows[2]}
      }
...
module.exports = {
 get_main_goes: getMain,
 get_stations_goes: getStations,
 get_hourly_actual_goes: getHourlyActual,
 get_hourly_goes:getHourlyGoes,
//query_all_stations_one_hour_actual
 //get_hourly_prediction: getHourlyPrediction,
 get_station_plot_goes: getStationPlot,
 get_date_goes:getdate,
